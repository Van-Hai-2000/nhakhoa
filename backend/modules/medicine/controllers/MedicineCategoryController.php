...
use common\models\medicine\MedicineCategoryImage;
use common\models\medicine\search\MedicineCategorySearch;
use Yii;
...
class MedicineCategoryController extends Controller
{
            ...
            if ($model->save()) {
                ...
                if ($newimage && $countimage > 0) {
                    foreach ($newimage as $image_code) {
                        ...
                        if ($imgtem) {
                            $nimg = new MedicineCategoryImage();
                            $nimg->attributes = $imgtem->attributes;
                            $nimg->id = NULL;
                            unset($nimg->id);
                            $nimg->medicine_category_id = $model->id;
                            if ($nimg->save()) {
                                if ($recount == 0) {
    ...
    public function actionUpdate($id)
    {
            ...
            if ($model->save()) {
                if (isset($orders_images) && $orders_images) {
                    foreach ($orders_images as $stt => $img) {
                        $imag = MedicineCategoryImage::findOne($img);
                        $imag->order = $stt;
                        $imag->save();
                    }
                }
                    ...
                    foreach ($newimage as $image_code) {
                        ...
                        if ($imgtem) {
                            ...
                            if ($image_code == $simg_id) {
                                $avatar = MedicineCategoryImage::findOne($image_code);
                            }
                        }
                    }
                }
                ...
                if ($avatar && count($avatar)) {
                    ...
                    if ($simg_id != $model->avatar_id) {
                        $imgavatar = MedicineCategoryImage::findOne($simg_id);
                        if ($imgavatar) {
        ...
        $images = MedicineCategory::getImages($id);
    ...
    public function actionDeleteImage($id)
    {
        if (Yii::$app->request->isAjax) {
            ...
            $image = MedicineCategoryImage::findOne($id);
            if ($image->delete()) {
                return ['code' => 200];
            }
        }
    ...
    public function actionUploadproductfilecrop()
    {
        ...
        $file = '../../static/media/images/medicine_category/' . $name;

        $success = file_put_contents($file, $data);

        if ($avatartg = MedicineCategoryImage::findOne($id)) {
            $avatartg->path = '/media/images/medicine_category/';
            $avatartg->name = $name;
            $avatartg->save();
        } else {
        ...
        return ClaHost::getImageHost() . '/media/images/medicine_category/s200_200/' . $name;
    }
