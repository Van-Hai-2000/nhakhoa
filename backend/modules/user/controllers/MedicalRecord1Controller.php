 ...
 */
class MedicalRecord1Controller extends Controller
{
    /**
     * @inheritdoc
     */
        ...
        if ($model -> load(Yii ::$app -> request -> post())) {
            ...
            try {
                ...
                foreach ($products as $k => $val) {
                    ...
                    $total_money += ($val['quantity'] * $product -> price) + ($val['quantity'] * $product -> price) * __VAT ;
                }
...
//            }
    ...
    public function actionLoadInfo()
    {
        $request = $_POST;
        $id = isset($request['id']) && $request['id'] ? $request['id'] : '';
        $info = MedicalRecordItem ::findOne($id);
        return $this -> renderPartial('layouts/info', [
            'info' => $info,
        ]);
    }
    ...
    public function actionUpdateForm()
    {
        ...
        $id = isset($request['model_id']) && $request['model_id'] ? $request['model_id'] : '';


        $doctor = isset($request['commission_doctor']) && $request['commission_doctor'] ? $request['commission_doctor'] : []; // danh sách bác sĩ
        $status = isset($request['commission_status']) && $request['commission_status'] ? $request['commission_status'] : []; //trạng thái
        $quantity = isset($request['commission_quantity']) && $request['commission_quantity'] ? $request['commission_quantity'] : []; // số lượng
        $money = isset($request['commission_money']) && $request['commission_money'] ? $request['commission_money'] : []; // giá tiền


        $model = MedicalRecordItemChild ::findOne($id);
